...
const loginShellProcesses = new Set(['-bash', '-fish', '-sh', '-tcsh', '-zsh']);
const isLoginShellProcess = process_ => loginShellProcesses.has(process_.name);

const isUninterestingProcess = process_ => isHelperProcess(process_) || isLoginShellProcess(process_);
...
const filterAndSortProcesses = (processes, term, searcher, flags) => {
	const filtered = processes.filter(process_ => !isUninterestingProcess(process_));
...
export {init, handleFkillError, isUninterestingProcess};
